		...
		<div class="pt-6 pb-16 sm:pb-24">
			<div class="mx-auto mt-8 max-w-2xl px-4 sm:px-6 lg:max-w-7xl lg:px-8">
				<div class="lg:grid lg:auto-rows-min lg:grid-cols-12 lg:gap-x-8">
                  ...
                  -->
										<label
											class="flex cursor-pointer items-center justify-center rounded-md border py-3 px-3 text-sm font-medium uppercase focus:outline-none sm:flex-1">
											<input
												type="radio"
												name="size-choice"
												value="XXS"
                  ...
                  -->
										<label
											class="flex cursor-not-allowed items-center justify-center rounded-md border py-3 px-3 text-sm font-medium uppercase opacity-25 sm:flex-1">
											<input
												type="radio"
												name="size-choice"
												value="XL"
...
export default {
	...
	methods: {
		async checkRedemption() {
			try {
				const response = await fetch('/.netlify/functions/has-redeemed', {
					method: 'POST',
					body: JSON.stringify({ ethAddress: this.$wallet.account }),
				})
...
input[type='text'] {
	@apply mt-1 block w-full rounded-md border-2 border-gray-300 bg-transparent p-2 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm;
}
</style>
