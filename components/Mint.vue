<template>
	<supply-check class="-mt-2 text-white">
		<div class="mt-12">
			<div
				...
				<div v-if="isPresaleActive && isPresaleVerified">
					<quantity-selector
						...
						:min="1"
						:max="PRESALE_LIMIT" />
					<pill-button @click.native="handleMint('presale')"
						>Whitelist Mint
					</pill-button>
				</div>
			...
			<div
				class="mx-auto mb-8 max-w-sm text-center"
				v-else>
				<div>
					<span v-if="isPublicSaleActive || isPresaleActive">
...
export default {
	...
	computed: {
		shouldShowMintButtons() {
			return (
				this.isPublicSaleActive || (this.isPresaleActive && this.isPresaleVerified)
			)
		},
		totalSupply() {
